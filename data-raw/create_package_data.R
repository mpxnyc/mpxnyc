...
neighborhood_sf_obj               <- sf::st_simplify(
                                                      neighborhood_sf_obj,
                                                      preserveTopology = T,
                                                      dTolerance = 200
                                                      )
...
shape_list                        <- lapply(
                                            shape_list,
                                            function(x) sf::st_transform(x = x, crs = sf::st_crs(census_tract_sf_obj))
                                            ) |>
                                            lapply(data.frame)
...
mpxnyc_colors[["dark_pink"]]      <- "#F73C95"
mpxnyc_colors[["light_pink"]]     <- "#FF99C5"
mpxnyc_colors[["dark_blue"]]      <- "#009BE8"
mpxnyc_colors[["mid_blue"]]       <- "#41BFFF"
mpxnyc_colors[["light_blue"]]     <- "#C5EFFF"
mpxnyc_colors[["dark_purple"]]    <- "#5D207A"
mpxnyc_colors[["mid_purple"]]     <- "#7F3F98"
mpxnyc_colors[["light_purple"]]   <- "#9E7ACC"
mpxnyc_colors[["dark_green"]]     <- "#459B0E"
mpxnyc_colors[["light_green"]]    <- "#59C618"
mpxnyc_colors[["dark_orange"]]    <- "#EA773D"
mpxnyc_colors[["light_orange"]]   <- "#FFAA7B"
mpxnyc_colors[["dark_brown"]]     <- "#844D28"
mpxnyc_colors[["mid_brown"]]      <- "#D89000"
mpxnyc_colors[["light_brown"]]    <- "#FFCD2E"
mpxnyc_colors[["all_colors"]]     <- unlist(mpxnyc_colors)


## DATA

mpxnyc_1_person_data               <- readRDS("data-raw/mpxnyc_1_person_data.R")
mpxnyc_1_place_data                <- readRDS("data-raw/mpxnyc_1_place_data.R")
mpxnyc_2_data                      <- readRDS("data-raw/mpxnyc_2_data.R")



# CREATE PACKAGE DATA

usethis::use_data(census_tract_sf_obj,                                                      internal=FALSE, overwrite=TRUE)
usethis::use_data(neighborhood_sf_obj,                                                      internal=FALSE, overwrite=TRUE)
usethis::use_data(borough_sf_obj,                                                           internal=FALSE, overwrite=TRUE)
usethis::use_data(mpxnyc_colors,                                                            internal=FALSE, overwrite=TRUE)
usethis::use_data(mpxnyc_1_person_data,                                                     internal=FALSE, overwrite=TRUE)
usethis::use_data(mpxnyc_1_place_data,                                                      internal=FALSE, overwrite=TRUE)
usethis::use_data(mpxnyc_2_data,                                                            internal=FALSE, overwrite=TRUE)
